...
public class LightCone : MonoBehaviour
{
    ...
    public List<GameObject> objectsHits = new List<GameObject>();
    ...
    void Update()
    {
        ...
        // Return List here per  update
        objectsHits.Clear();
        
        foreach (var ray in GetRaycastHits())
        {
            // Currently setting to player tag only
            if (ray.collider.tag == "Player")
            {
                objectsHits.Add(ray.transform.gameObject);
            }
        }
    ...
    private void OnDrawGizmos()
    {
        ...
        if (alwaysDrawGizmos) OnDrawGizmosSelected(false);
    ...
    void OnDrawGizmosSelected(bool isSelected = true)
    {
        if (Application.isPlaying) return;
        ...
        Color unselectedColor = Color.Lerp(Color.clear, color, 0.75f);
